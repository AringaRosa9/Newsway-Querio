  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex h-1.5 flex-1 rounded-full overflow-hidden bg-gray-100">
        {pPos > 0 && (
          <div className="bg-green-400" style={{ width: `${pPos}%` }} />
        )}
        {pNeu > 0 && (
          <div className="bg-gray-300" style={{ width: `${pNeu}%` }} />
        )}
        {pNeg > 0 && (
          <div className="bg-red-400" style={{ width: `${pNeg}%` }} />
        )}
      </div>
    ...
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="p-5">
        <div className="flex items-start justify-between mb-2">
          ...
          <span className="ml-3 flex-shrink-0 px-2 py-0.5 text-xs bg-blue-50 text-blue-700 border border-blue-100 rounded-full">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          {categoryLabel && (
            <span className="px-2 py-0.5 text-xs bg-purple-50 text-purple-700 border border-purple-100 rounded-full">
              {categoryLabel}
            </span>
          )}
        ...
        {event.entities.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-3">
            {event.entities.slice(0, 8).map((entity, i) => (
              <span
                key={i}
                className="px-2 py-0.5 text-xs bg-gray-50 text-gray-600 border border-gray-100 rounded"
              >
                {entity}
              </span>
            ))}
